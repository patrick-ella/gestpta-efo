...
import { ChevronDown, ChevronRight, Copy, Loader2, Package, CheckCircle2, AlertTriangle, Lock, Pencil } from "lucide-react";
...
const PtaTreeView = ({ activites, isAdmin, onSelectSousTache, onRefresh }: PtaTreeViewProps) => {
  ...
  const handleDuplicate = async (act: PtaActivite) => {
    ...
    try {
      ...
      const { data: newAct, error: actErr } = await supabase.from("activites").insert({ exercice_id: nextEx!.id, code: newCode, libelle: act.libelle, objectif_operationnel: act.objectif_operationnel, budget_total: act.budget_total, ordre: act.ordre }).select("id").single();
      if (actErr) throw actErr;
      for (const tache of act.taches) {
        const tacheCode = tache.code + "_" + nextYear;
        const { data: newTache, error: tErr } = await supabase.from("taches").insert({ activite_id: newAct!.id, code: tacheCode, libelle: tache.libelle, livrables: tache.livrables, budget_total: tache.budget_total, ordre: tache.ordre }).select("id").single();
        if (tErr) throw tErr;
  ...
  // Check if item was recently edited (within 7 days)
      ...
      {activites.map((act) => {
        ...
        return (
          <div key={act.id} className="rounded-lg border overflow-hidden">
            {/* Activity row */}
            <div
              className="flex items-center justify-between px-4 py-3 bg-primary text-primary-foreground cursor-pointer select-none group"
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                {isAdmin && (
                  <>
                    ...
                    </TooltipTrigger><TooltipContent>Modifier le libellé (double-clic)</TooltipContent></Tooltip>
                    <Tooltip><TooltipTrigger asChild>
            ...
            {actExpanded && act.taches.map((tache) => {
              ...
              return (
                <div key={tache.id}>
                  <div
                    className="flex items-center justify-between px-6 py-2.5 bg-secondary text-secondary-foreground cursor-pointer select-none border-t border-secondary/50 group/tache"
                  ...
                  {tacheExpanded && tache.sous_taches.map((st, idx) => (
                    <div
                      key={st.id}
                      className={`flex items-center justify-between px-10 py-2 cursor-pointer hover:bg-accent/50 border-t transition-colors ${idx % 2 === 0 ? "bg-light-blue" : "bg-light-blue-row"}`}
